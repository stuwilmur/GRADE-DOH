...
function getProjectionData(
  ...
  for (
    let y = _firstyear;
    y <= popdata.lastyear && y - _firstyear <= years_to_project;
    y++
  ) {
    ...
    if (ret.error === null) {
      ...
      datarow.additional = computeAdditionalResults(
        _country,
        y,
        _outcome,
        NaN,
      ...
      datarow.special = computeSpecialResults(
        _country,
        y,
        _outcome,
        NaN,
  ...
  if (
    _smooth &&
    _governance.model == 'EXOGENOUS' &&
    years_successful > years_to_wait
  ) {
...
function getProjectionCSVData(
  ...
  for (let iCountry = 0; iCountry < _countries.length; iCountry++) {
    ...
    _outcomes.forEach(function (thisOutcome) {
      let projectionData = getProjectionData(
        _year,
        _country,
        thisOutcome,
        _years_to_project,
      ...
      if (projectionData.error) {
        ...
        ret.errors.push(
          countrycodes.get(_country) + ': ' + projectionData.error,
        );
      }
    ...
    data.forEach(function (datarow) {
      let row = '';
      body +=
        countrycodes.get(_country).replace(',', ' -') +
        ',' +
        _country +
        ',' +
        datarow.year +
...
function calcProjection(
  ...
  var plotdata = getProjectionData(
    _year,
    _country,
    _outcome,
    _years_to_project,
  ...
  plotdata.data.forEach(function (d) {
    d.additional.forEach(function (result) {
      if (totals.has(result.name)) {
        ...
        totals.set(result.name, {
          projection: totals.get(result.name).projection + result.value,
          effect: totEffectPeriod,
        });
      } else {
  ...
  if (theOutcome.isStockVar) {
    ...
    totals.forEach(function (value, result) {
      var result_avg_effect =
        plotdata.years_of_effect > 0
          ? value.effect / plotdata.years_of_effect
          : 0;
      var result_avg_projection = value.projection / plotdata.data.length;
      averages.set(result, {
        projection: result_avg_projection,
        effect: result_avg_effect,
      });
    });
