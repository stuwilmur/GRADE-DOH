...
var selections = new Map([
  ...
  [
    '$-LIC',
    {
      ...
      short_desc: 'LICs',
      fn: (_cid, _year) =>
        popdata.getstring(_cid, _year, 'incomelevel') == 'LIC',
    },
  ],
  [
    '$-LMIC',
    {
      ...
      short_desc: 'LMICs',
      fn: (_cid, _year) =>
        popdata.getstring(_cid, _year, 'incomelevel') == 'LMIC',
    },
  ],
  [
    '$-UMIC',
    {
      ...
      short_desc: 'UMICs',
      fn: (_cid, _year) =>
        popdata.getstring(_cid, _year, 'incomelevel') == 'UMIC',
    },
  ],
  [
    '$-HIC',
    {
      ...
      short_desc: 'HICs',
      fn: (_cid, _year) =>
        popdata.getstring(_cid, _year, 'incomelevel') == 'HIC',
    },
  ],
  [
    '$-REG-LCN',
...
function makeText2(
  ...
  text =
    text +
    "<h1 class='tooltip'> " +
    countrycodes.get(_iso) +
    '</h1>' +
    '<br/><strong>Projection for ' +
    _year +
    ' - ' +
    end_year +
    '</strong>';
  ...
  if (projection.error) {
    text =
      text + '<br/> Projection unavailable: ' + projection.error.join('<br>');
  } else {
    ...
    if (revenues === undefined) {
      ...
      text =
        ...
        _year +
        '</strong>' +
        "<br/>Current Gov. rev. per capita: <span class = 'ar'>$" +
        d3.format(',')(revenues['historical grpc'].toFixed(2)) +
        '</span>' +
        "<br/>New Gov. rev. per capita: <span class = 'ar'>$" +
        d3.format(',')(revenues['new grpc'].toFixed(2)) +
        '</span>' +
        "<br/>Increase in Gov. rev. per capita: <span class = 'ar'>" +
        (revenues['percentage increase'] * 100).toFixed(2) +
        '%</span>' +
        '<br><br>' +
        "<div class = 'cumulative'>" +
        '<strong>' +
        varText +
        ' effect, after start of effect in ' +
        projection.start_of_effect +
        ':</strong>';
      ...
      projection.data.forEach(function (value, result) {
        text +=
          ...
          result +
          ":<br/> <span class = 'ar'>" +
          d3.format(',')(value.effect.toFixed(0)) +
          '</span>';
      });
      text += '</div>';
    }
...
function makeText(_iso, _year, _revenue, _governance) {
  ...
  if (!allIndicatorsSelected()) {
    result = computeResult(
      ...
      outcome,
      revenues['new grpc'],
      revenues['historical grpc'],
      _governance,
    );
    coverageDescriptor = outcomesMap.get(outcome).isPercentage
      ? '% coverage'
      : 'coverage ratio';
  ...
  text +=
    '<br/><br/><strong>' +
    outcomesMap.get(outcome).name +
    '</strong>' +
    '<br/>(instantaneous effect)' +
    '<br/>Current ' +
    coverageDescriptor +
    ": <span class = 'ar'>" +
    result.original.toFixed(dp) +
    '</span>' +
    '<br/>New ' +
    coverageDescriptor +
    ": <span class = 'ar'>" +
    result.improved.toFixed(dp) +
    '</span>';
  ...
  if (result.hasOwnProperty('additional')) {
    result.additional.forEach(function (property) {
      text =
        ...
        '<br/>' +
        property.name +
        ":&nbsp&nbsp<span class = 'arb'>" +
        d3.format(',')(property.value.toFixed(0)) +
        '</span>';
    });
  }
  ...
  if (result.hasOwnProperty('special')) {
    result.special.forEach(function (property) {
      ...
      text =
        ...
        property.name +
        ":&nbsp&nbsp<span class = 'arb'>" +
        d3.format(formatter)(property.value) +
        '</span>';
    });
  }
...
function makeTextTarget(_year, _iso, _outcome, _target, _revenue) {
  ...
  var rev_add =
    (revenues['historical total revenue'] * result.grpc) / grpc_orig -
    revenues['historical total revenue'];
  var str =
    '<strong>' +
    countrycodes.get(_iso) +
    '&nbsp;' +
    _year +
    '<\/strong><br/>' +
    "Absolute additional revenue:<span class = 'ar'>$" +
    d3.format(',')(rev_add.toFixed(0)) +
    '</span><br/>' +
    "Additional revenue per cap.:<span class = 'ar'>$" +
    d3.format(',')(grpc_add.toFixed(2)) +
    '</span><br/>' +
    "Increase as % of gov. rev. per cap.:<span class = 'ar'>" +
    grpc_inc.toFixed(2) +
    '%</span><br/>';
...
function setupMenus(_countries, _outcomes) {
  function initMenus(countries, outcomes) {
    ...
    selections.forEach(function (selection, id) {
      selection_countries.push({id: id, name: selection.desc});
    });
    ...
    d3.select('#multicountrylist')
      .on('change', function (d) {
        if (
          !multiplecountries.includes(this.options[this.selectedIndex].value)
        ) {
    ...
    d3.select('#methodlist').on('change', function (d) {
      method = this.options[this.selectedIndex].value;
      if (method == 'percentage') {
        d3.select('#revDiv').style('display', 'block');
        d3.select('#absRevDiv').style('display', 'none');
        d3.select('#pcRevDiv').style('display', 'none');
        d3.select('#prefix').style('display', 'none');
        d3.select('#grpcdiv').style('display', 'none');
      } else if (method == 'pc') {
        d3.select('#revDiv').style('display', 'none');
        d3.select('#absRevDiv').style('display', 'none');
        d3.select('#pcRevDiv').style('display', 'block');
        d3.select('#prefix').style('display', 'none');
        d3.select('#grpcdiv').style('display', 'none');
        ...
        d3.select('#pcRevDiv').style('display', 'none');
        d3.select('#prefix').style('display', 'none');
        d3.select('#grpcdiv').style('display', 'block');
      } else {
        d3.select('#revDiv').style('display', 'none');
        d3.select('#absRevDiv').style('display', 'block');
        d3.select('#pcRevDiv').style('display', 'none');
        d3.select('#prefix').style('display', 'block');
        d3.select('#grpcdiv').style('display', 'none');
      }
      if (method == 'file') {
        d3.select('#revenueFileInput').style('display', 'block');
        d3.select('#revenueStandardInput').style('display', 'none');
      } else {
        d3.select('#revenueFileInput').style('display', 'none');
        d3.select('#revenueStandardInput').style('display', 'block');
      }
      mainUpdate();
    });

    d3.select('#prefix').on('change', function (d) {
      ...
      d3.select('#absRevenueVal').text(
        '$' + Math.round(absGovRev / prefixValue) + getPrefix(prefix),
      );
      mainUpdate();
    });

    d3.select('#filemethod').on('change', function (d) {
  ...
  d3.select('#absRevSlider').on('input', function (d) {
    ...
    d3.select('#absRevenueVal').text(
      '$' +
        d3.format(',')(Math.round(absGovRev / getPrefixValue(prefix))) +
        getPrefix(prefix),
    );
    mainUpdate();
  });
  ...
  d3.select('#pcRevSlider').on('input', function (d) {
    ...
    d3.select('#perCapitaRevenueVal').text(
      '$' + d3.format(',')(Math.round(pcGovRev)),
    );
    mainUpdate();
  });
  ...
  d3.select('#govSlider').on('input', function (d) {
    governance =
      Math.round(
        ((this.value / numGovernanceSteps) * maxGovernance - maxGovernance) *
          100,
      ) / 100;
  ...
  d3.selectAll("input[name='governance model']").on('change', function () {
    ...
    document.getElementById('smooth').style.display =
      governanceModel == 'EXOGENOUS' ? 'block' : 'none';

    // Disable the governance slider if using the endogenous model
    document.getElementById('govSlider').disabled =
      governanceModel == 'ENDOGENOUS';
    document.getElementById('governancepanel').style.opacity =
      governanceModel == 'ENDOGENOUS' ? 0.5 : 1.0;
    // Disable the target SDG controls if using the endogenous model
    document.getElementById('targetInput').disabled =
      governanceModel == 'ENDOGENOUS';
    document.getElementById('targetpanel').style.opacity =
      governanceModel == 'ENDOGENOUS' ? 0.5 : 1.0;
    mainUpdate();
  });
  ...
  document.getElementById('gov controls').style.display = 'block';
  document.getElementById('gov quality').style.display = 'block';
...
function updateCountries() {
  ...
  if (!allIndicatorsSelected()) {
    ...
    d3.select('#countrytext').html(text);
    d3.select('#countrydata').style(
      'display',
      text.length > 0 ? 'inline-block' : 'none',
    );
  } else {
  ...
  var outcomedata = {
    ...
    text: data.map(function (a) {
      ...
      a.gov.forEach(function (gov_result, k) {
        s += gov_result.desc + ' ' + gov_result.value.toFixed(2) + '<br>';
      });
      return s;
    }),
    type: 'scatter',
...
function getErrorTextFromPictogramData(errors) {
  return errors.filter((e) => e !== null && e.length > 0).length > 0
    ? 'Unable to calculate some results (e.g. due to missing data)'
    : '';
}
...
function convertProjectionDataToPictogramData(data) {
  ...
  data.forEach(function (outcomeData, outcomeIndex) {
    ...
    var finalResultThisOutcomeDataSeries =
      thisOutcomeDataSeries[thisOutcomeDataSeries.length - 1];
    const value =
      finalResultThisOutcomeDataSeries.additional[0].value.toFixed(0);
    finalResults.push(value);
    populationName =
      finalResultThisOutcomeDataSeries.additional[0].populationName;
    ...
    if (
      finalResultThisOutcomeDataSeries.hasOwnProperty('special') &&
    ...
    if (
      hasCoverageValueReachedSaturation(
        finalResultThisOutcomeDataSeries.coverage[0].value,
        outcomeData.outcome,
      )
    ) {
  ...
  if (saturationWarning) {
    warnings.push(
      'The base value for at least one indicator saturates by the end of the projection period: population results may fall to zero',
    );
  }
...
function getPictogramProjectionData() {
  ...
  selectedOutcomesList.forEach(function (outcomeName) {
    ...
    var thisOutcomeProjectionData = getProjectionData(
      +year,
      country,
      outcomeName,
      +years_to_project,
    ...
    projectionData.push({
      ...
      data: thisOutcomeProjectionData,
    });
  });
...
function updateplot() {
  ...
  if (multipleCountriesSelected()) {
    ...
    // Show the plot
    let plotdata = getProjectionData(
      +year,
      country,
      outcome,
      +years_to_project,
    ...
    if (
      data.some((d) =>
        hasCoverageValueReachedSaturation(d.coverage[0].value, outcomeObject),
      )
    ) {
      d3.select('#plot-errors').html(
        getCoverageSaturationWarningText(outcomeObject),
      );
    }

    var dataFromObservation = data.filter(
      (x) => x.year <= startingYearOfExtensions,
    );
    ...
    plotlayout.title =
      'Projection for ' + countrycodes.get(country) + ': ' + theOutcome.name;
...
function updatetarget() {
  ...
  d3.select('#targetText').html(text);
}
...
function download_csv(
  ...
  var csvdata = getProjectionCSVData(
    _year,
    countries,
    _outcomes,
    _years_to_project,
  ...
  if (csvdata.str) {
    var hiddenElement = document.createElement('a');
    hiddenElement.href =
      'data:text/csv;charset=utf-8,' + encodeURI(csvdata.str);
    hiddenElement.target = '_blank';
    hiddenElement.download = button_title;
    hiddenElement.click();
  }
...
function download_csv_multi() {
  ...
  var countries_to_export = multiplecountries.filter(
    (d) => !isCountrySpecialSelection(d),
  );
  var special_selections = multiplecountries.filter((d) =>
    isCountrySpecialSelection(d),
  );
  ...
  var error = download_csv(
    +year,
    +years_to_project,
    countries_to_export.sort(),
    outcomes,
    getRevenueInputs(),
    getGovernanceInputs(),
    smooth,
  ...
  if (error) {
    d3.select('#multicountryerror').html(error.join('<br />'));
  }
}

function updateLegend() {
  var theOutcome = outcomesMap.get(outcome);
  var domain = theOutcome.hasOwnProperty('fixedExtent')
    ? theOutcome.fixedExtent
    : theOutcome.extent;
  colorScale.range([theOutcome.loCol, theOutcome.hiCol]).domain(domain);
  ...
  svg2.select('.legendLinear').call(legendLinear);

  d3.select('#legend-label').html(theOutcome.desc);
}
...
function loaded(error, _countries, _popdata, _gdpdef) {
  ...
  var theOutcome = outcomesMap.get(outcome);
  colorScale.domain(
    theOutcome.hasOwnProperty('fixedExtent')
      ? theOutcome.fixedExtent
      : theOutcome.extent,
  );

  var _countries = topojson.feature(
    _countries,
    _countries.objects.units,
  ).features;
  _countries.forEach(function (d) {
    countrycodes.set(d.id, d.properties.name);
  });

  var popcountries = popdata.nesteddata.map(function (d) {
    return {id: d.key, name: d.values[0].values[0].countryname};
  });
  popcountries.forEach(function (d) {
    countrycodes.set(d.id, d.name);
  });
  ...
  d3.select('#legend-label').html(outcomesMap.get(outcome).desc);
...
function updateCountryFilters() {
  ...
  function gethtml(d) {
    ...
    selections.forEach(function (d, k) {
      country_and_selection_codes.set(k, d.short_desc);
    });
  ...
  d3.select('#multicountryerror').html('');
...
function set_outcome_target() {
  ...
  if (allIndicatorsSelected() || multipleCountriesSelected()) {
    d3.select('#targetInput').property('value', 100).property('disabled', true);
    return;
  }
  ...
  d3.select('#targetInput')
    .property('value', target_value.toFixed(dp))
    .property('disabled', false)
    .style('box-shadow', '0 0 5px #ffdb8d')
    .style('background-color', '#ffdb8d');
  target = target_value;
  d3.select('#targetLabel').text(
    outcomesMap.get(outcome).isPercentage ? '%' : '',
  );
}
