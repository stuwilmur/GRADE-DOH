...
function getGov(_type, _iso, _year, _gov, _grpc = 0) {
  ...
  if (_gov.model == 'EXOGENOUS') {
    ...
    x =
      pop[_type] +
      (govMeasures.get(_type).positive == true ? _gov.value : -_gov.value);
  } else if (
    _gov.model == null ||
    (_gov.model == 'ENDOGENOUS' && _gov.table == null)
  ) {
    /*
  ...
  for (let i = 0; i < _yearsToForecast; i++) {
    ...
    if (i == 0) {
      ...
      govMeasures.forEach(function (value, measure) {
        ...
        var measure_lagged2 =
          gov_lagged2 == null ? null : gov_lagged2.get(measure);
        var measureEquationForecast = govMeasures
          .get(measure)
          .fn(
            pop[measure],
        ...
        // On timestep 1, pass -1 for the unavailable second lagged meausre
        var measureImproved_lagged2 =
          govImproved_lagged2 == null ? null : govImproved_lagged2.get(measure);
        var measureWithIncreasedGovRev = govMeasures
          .get(measure)
          .fn(
            pop[measure],
